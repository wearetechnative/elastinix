# Spec: jiraticketcreate-safe-json

## Purpose

Defines requirements for safe JSON assembly in the `jiraticketcreate` service script, ensuring all field values are correctly escaped and that `jq` is referenced by its absolute Nix store path.
