...
Defines how labels are applied to Jira tickets created by the `jiraticketcreate` module. A fixed module label (`jiraticketcreate-elastinix`) is always added to every ticket for traceability, and each `checkType` can optionally supply additional labels that are appended after the fixed label.
