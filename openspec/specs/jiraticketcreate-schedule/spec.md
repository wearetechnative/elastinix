# Spec: jiraticketcreate-schedule

## Purpose

Defines the scheduling interface for the `jiraticketcreate` service. A single unified `schedule` option replaces the former `frequency` and `timerCalendar` options, supporting both structured frequency strings (with bash day-checking logic) and arbitrary systemd calendar expressions.
