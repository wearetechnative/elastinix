...
> **Note:** This service is supported on the `nixos-26.05` elastinix branch only.
> Its OIDC secrets are read via systemd credentials (`clientSecretFile` /
> `cookieSecretFile`), which require nixpkgs 26.05; the `nixos-25.11` branch does
> not carry it. The `grafana-prometheus` flake input is pinned to a
> `wearetechnative/monitoring` revision that includes the `oauth2Proxy` option
> surface.
